...
public class ModuloEmocional {
    ...
    public String separarRespuestaGPT(String respuesta){
        ...
        Log.d("Respuesta GPT original", respuesta);
    ...
    private void sentimientosRobot(String emocionesRobot) throws InterruptedException {
        ...
        Log.d("emocionesRobot", emocionesRobot);
    ...
    private void expresividadGestual(List<Integer> codigoEmocionesRobot) throws InterruptedException {
        ...
        else if(indexSentimiento>=3 && indexSentimiento<=6){ // CONFIANZA
            systemControl.cambiarEmocion(EmotionsType.PRISE);
        }
        else if(indexSentimiento>=7 && indexSentimiento<=9){ // MIEDO
            systemControl.cambiarEmocion(EmotionsType.GRIEVANCE);
            hardwareControl.encenderLED(LED.PART_ALL, LED.MODE_PURPLE);
            Thread.sleep(3000);
            hardwareControl.apagarLED(LED.PART_ALL);
        }
        else if(indexSentimiento>=10 && indexSentimiento<=11){ // SORPRESA
            systemControl.cambiarEmocion(EmotionsType.SURPRISE);
            handsControl.controlBasicoBrazos(HandsControl.AccionesBrazos.LEVANTAR_BRAZO,  HandsControl.TipoBrazo.AMBOS);
            Thread.sleep(3000);
            handsControl.reiniciar();
        }
        else if(indexSentimiento==12){ // DISTRACCIÓN
            systemControl.cambiarEmocion(EmotionsType.SWEAT);
        }
        else if(indexSentimiento>=13 && indexSentimiento<=14){ // TRISTEZA
        ...
        else if(indexSentimiento==27){ // SUMISION
            systemControl.cambiarEmocion(EmotionsType.GRIEVANCE);
        }
        else if(indexSentimiento==28){ // SUSTO
            systemControl.cambiarEmocion(EmotionsType.SWEAT);
        }
        else if(indexSentimiento==29){ // DECEPCION
            systemControl.cambiarEmocion(EmotionsType.GOODBYE);
        }
        else if(indexSentimiento==30){ // REMORDIMIENTO
            systemControl.cambiarEmocion(EmotionsType.SWEAT);
        }
        else if(indexSentimiento==31){ // DESPRECIO
            systemControl.cambiarEmocion(EmotionsType.ARROGANCE);
        }
        else if(indexSentimiento==32){ // AGRESIVIDAD
            systemControl.cambiarEmocion(EmotionsType.ANGRY);
        }
        else if(indexSentimiento==33){ // ESPERANZA
            systemControl.cambiarEmocion(EmotionsType.SMILE);
        }
        else if(indexSentimiento==34){ // CULPA
            systemControl.cambiarEmocion(EmotionsType.SWEAT);
        }
        else if(indexSentimiento==35){ // CURIOSIDAD
            systemControl.cambiarEmocion(EmotionsType.SNICKER);
        }
        else if(indexSentimiento==36){ // DESESPERACION
            systemControl.cambiarEmocion(EmotionsType.GRIEVANCE);
        }
        else if(indexSentimiento==37){ // INCREDULIDAD
            systemControl.cambiarEmocion(EmotionsType.SURPRISE);
        }
        else if(indexSentimiento==38){ // ENVIDIA
            systemControl.cambiarEmocion(EmotionsType.ARROGANCE);
        }
        else if(indexSentimiento==39){ // CINISMO
            systemControl.cambiarEmocion(EmotionsType.ARROGANCE);
        }
        else if(indexSentimiento==40){ // ORGULLO
            systemControl.cambiarEmocion(EmotionsType.SMILE);
        }
        else if(indexSentimiento==41){ // ANSIEDAD
            systemControl.cambiarEmocion(EmotionsType.GRIEVANCE);
        }
        else if(indexSentimiento==42){ // DELEITE
            systemControl.cambiarEmocion(EmotionsType.SMILE);
        }
        else if(indexSentimiento==43){ // SENTIMENTALISMO
            systemControl.cambiarEmocion(EmotionsType.SMILE);
        }
        else if(indexSentimiento==44){ // VERGUENZA
        ...
        else if(indexSentimiento==45){ // INDIGNACIÓN
            systemControl.cambiarEmocion(EmotionsType.ABUSE);
        }
        else if(indexSentimiento==46){ // PESIMISMO
            systemControl.cambiarEmocion(EmotionsType.GOODBYE);
            headControl.controlBasicoCabeza(HeadControl.AccionesCabeza.ABAJO);
            Thread.sleep(3000);
            headControl.reiniciar();
        }
        else if(indexSentimiento==47){ // MORBOSIDAD
            systemControl.cambiarEmocion(EmotionsType.FAINT);
        }
        else if(indexSentimiento==48){ // DOMINANCIA
            systemControl.cambiarEmocion(EmotionsType.ARROGANCE);
        }
    }
