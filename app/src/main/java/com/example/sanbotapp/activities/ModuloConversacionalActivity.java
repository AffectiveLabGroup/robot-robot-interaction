...
import com.example.sanbotapp.modulos.moduloReactive.RecognitionControl;
...
public class ModuloConversacionalActivity extends TopBaseActivity {
    ...
    private String responseModelExpression = "";
    private String contentConversacion = "quiero que mantengamos una conversación";

    private String responseUser = "detecta en la respuesta que te voy a dar las siguientes caracteristicas de la persona, la edad y" +
            " el nombre. Quiero que a veces me llames por mi nombre y que adaptes la conversación teniendo en cuenta la edad mencionada, si detectas que cambia mi nombre, pregunta siempre por mi edad nuevamente y" +
            "que adaptes la conversación teniendo en cuenta la edad mencionada de la nueva persona. " + "Enlazame al final de todo el texto separado con un coma (,) el nombre del usuario y la edad, " +
            "y que este enmarcado con simbolos de mayor que y menor que, seria de esta manera, por ejemplo: <nombre del usuario,edad del usuario>, en caso de que no detectes el nombre del usuario especificalo como" +
            "Anonimo";
    ...
    private Handler handlerQuestion = new Handler();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        clasificarRoleSystem(contentPersonalizacion, contentContextualizacionSinContexto, contentContextualizacionConContexto, responseUser);

        //Facial Expresion Recognition
        tvMedia = findViewById(R.id.tv_media);

        tvMedia.setVisibility(View.VISIBLE);
        tvMedia.bringToFront();


        recognitionControl = new RecognitionControl(speechManager, mediaManager, tvMedia, getApplicationContext());
        ...
        try {
            ...
            handlerQuestion = new Handler();
            // Configura el handler para que vuelva a ejecutar este bloque en 120 segundos
            runnable = new Runnable() {
                @Override
                public void run() {
                    if(nombreUsuario == null || nombreUsuario.isEmpty() || nombreUsuario.isBlank() || nombreUsuario.equals("Anonimo")) nombreUsuario=null;
                    // Preguntar aleatoriamente
                    int random = (int) (Math.random() * 3);
                    if (random == 0) {
                        speechControl.hablar("Hola," + nombreUsuario + "¿estas ahi?");
                        while (speechControl.isRobotHablando()) {
                        }
                        // Poner a la escucha
                        speechManager.doWakeUp();
                    } else if (random == 1) {
                        speechControl.hablar("¿Quieres continuar conversando?" + nombreUsuario);
                        while (speechControl.isRobotHablando()) {
                        }
                    } else {
                        speechControl.hablar("Hola, " + nombreUsuario + "¿Quieres que hablemos de otro tema?");
                        while (speechControl.isRobotHablando()) {
                        }
                        // Poner a la escucha
                        speechManager.doWakeUp();
                    ...
                    // Configura el handler para que vuelva a ejecutar este bloque en 120 segundos
                    handlerQuestion.postDelayed(this, 120000);
                }
            };

            // Iniciar el ciclo de preguntas después de una pequeña demora inicial (opcional)
            handlerQuestion.postDelayed(runnable, 120000);
    ...
    private void executeRecognition(){
        ...
        executor.execute(() -> {
            Log.d("respuesta", "entro al  executeRecognition");
            recognitionControl.recognitionExpresion();

            // Para volver a la UI
            new Handler(Looper.getMainLooper()).post(() -> {
                Log.d("respuesta", "entro al  Handler");
                responseModelExpression = recognitionControl.responseModelExpression + " / ";
                textViewEmotion.append(responseModelExpression);
                Log.d("respuesta", "salio del Handler");
            });
        });
            ...
            public void run(){
                ...
                handler.post(new Runnable() {
                    public void run() {
                        ...
                        try {
                            handler.removeCallbacksAndMessages(null);
                            handlerQuestion.removeCallbacks(runnable);
    ...
    private void clasificarRoleSystem(String contentPersonalizacion, String contentContextualizacionSinContexto, String contentContextualizacionConContexto, String responseUser){
        // PERSONALIZACIÓN + CONVERSACIÓN + INTERPRETACIÓN EMOCIONAL + CONTEXTUALIZACIÓN CON CONTEXTO

        if(isAutomatic) {
            moduloOpenAI.anadirRoleSystem(contentConversacion + "," + contentInterpretacionEmocional + responseUser);
        }
    ...
    private void firstQuestion(){
        gestionarModulosConversacion();
        //vozSeleccionada = "nova";
        if(!personalizacionUsuarioActivada){
            speechControl.hablar("Hola, como estas!!!, por favor me pudiera decir su nombre y edad.");
        }
    }
    ...
    private void setNameYearsUser(){
        try {
            String texto = getRespuestaGPT();
            respuestaGPT = respuestaGPT.substring(0, texto.indexOf("<")-1);
            // Eliminamos los caracteres '<' y '>'
            String contenido = texto.substring(texto.indexOf("<")+1, texto.indexOf(">"));
            // Dividimos la cadena por la coma
            String[] partes = contenido.split(",");

            // Guardamos el nombre y la edad en variables
            nombreUsuario = partes[0];
            edadUsuario = Integer.parseInt(partes[1]);

            // Imprimimos los resultados
            Log.d("datos user","Nombre: " + nombreUsuario + "Edad: " + edadUsuario);
        } catch (Exception e) {
            Log.d("getNameYearsUser", "Error obteniendo nombre y edad");
        }
    }
